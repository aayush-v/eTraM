...
class eTraM(EventDataModule):

    def __init__(self, batch_size: int = 64, shuffle: bool = True, num_workers: int = 8, pin_memory: bool = False,
                 transform: Optional[Callable[[Data], Data]] = None):
        super(eTraM, self).__init__(img_shape=(1280, 720), batch_size=batch_size, shuffle=shuffle,
    ...
    def _processing(self, rf: str, root: str, read_annotations, read_label, buffer_to_data):
        ...
        for i, bbox in enumerate(bounding_boxes):
            ...
            ######### MIN NUMBER OF EVENTS TO QUALIFY AS A VALID GRAPH: NEED TO PLAY AROUND WITH DIFFERENT VALUES #########
            if data.size < 50_000:
                print('skipping..', t_bbox)
                continue
